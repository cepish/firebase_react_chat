...
const Messages: React.FC<IProps> = props => {
  ...
  return (
    <ScrollChat className={css.container}>
      <div className={css.etx}>That&#39;s every message!</div>
      {messages.map((message: IChannelMessage, index: number, all: IChannelMessage[]) => {
        ...
        const showDay =
          isFirstMessage || (messagesExist && shouldShowDate(previous, message))

        const showAvatar =
          isFirstMessage || (messagesExist && shouldShowAvatar(previous, message))
