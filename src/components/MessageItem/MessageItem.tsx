...
const MessageItem: React.FC<IProp> = props => {
  ...
  const getMessagesDate = (date: string): string =>
  ...
  const messageWithAvatar = author ? (
    <div className={classNames(css.message, css.withAvatar)}>
      ...
      <div className={css.author}>
        <div>
          <span className={css.userName}>{author.displayName}</span>
          <span className={css.timeStamp}>{getMessageTime(message.createdAtStr)}</span>
        </div>
  ...
  const messageItem = showAvatar ? messageWithAvatar : messageWithNoAvatar
